...
@section('page_title', __('general.manage_timetables'))
@section('content')

    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title">{{ __('general.manage_timetables') }}</h6>
            {!! Qs::getPanelOptions() !!}
        </div>
        ...
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                @if(Qs::userIsTeamSA())
                <li class="nav-item"><a href="#add-tt" class="nav-link active" data-toggle="tab">{{ __('general.create_timetable') }}</a></li>
                @endif
                <li class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">{{ __('general.show_timetables') }}</a>
                   ...
                   <div class="col-md-8">
                       <form class="ajax-store" method="post" action="{{ route('ttr.store') }}">
                           ...
                           <div class="form-group row">
                               ...
                               <div class="col-lg-9">
                                   <input name="name" value="{{ old('name') }}" required type="text" class="form-control" placeholder="Nome do Horário">
                               </div>
                           </div>
                           ...
                           <div class="form-group row">
                               <label for="exam_id" class="col-lg-3 col-form-label font-weight-semibold">Tipo (Turma ou Avaliação)</label>
                               <div class="col-lg-9">
                    ...
                    <div class="tab-pane fade" id="ttr{{ $mc->id }}">                         <table class="table datatable-button-html5-columns">
                            ...
                            @foreach($tt_records->where('my_class_id', $mc->id) as $ttr)
                                <tr>
                                    ...
                                    <td>{{ ($ttr->exam_id) ? $ttr->exam->name : 'Turma' }}</td>
                                    <td>{{ $ttr->year }}</td>
